...
const isTweetTaken = (tweet) => {
    return postedTweets.includes(tweet);
};
...
const generateTweet = (categories, calendar) => {
    ...
    if (readFromConfig(conf.default.aida.enableReposts) == true) {
        ...
        if (!isTweetTaken(randomTweet.text)) {
            ...
            logger.debug(
                `Proposed posting date chosen from preferred posting intervals: ${postingDate}`
            );
    ...
    if (acceptedPostingPeriods.includes(postingPeriod)) {
        switch (postingPeriod) {
            case 'weekly':
                ...
                lastPostDay = set(endOfWeek(startingDate), {
                    hours: 23,
                    minutes: 59,
                    seconds: 59,
                    milliseconds: 999,
        ...
        throw new Error(
            `Calendar is unable to set posting period! postingPeriod within config.json must be either: ${JSON.stringify(
                acceptedPostingPeriods
            )}.`
        );
    }
...
const generatePreferredTimes = (day) => {
    ...
    const timeWithinRange = () => {
        ...
        try {
            logger.debug(
                `Attempting to read posting interval for day ${format(
                    day,
                    'i'
                )} from ${JSON.stringify(
                    readFromConfig(
            ...
            interval = readFromConfig(
                conf.default.calendar.preferredPostingInterval[format(day, 'i')]
            );
        } catch {
            logger.debug(
                `Interval ${JSON.stringify(
                    day
                )} is outside preferred posting interval. Using the posting interval fallback...`
            );
        }
...
const generatePreferredDate = (calendar) => {
    ...
    logger.debug(
        `Post chance: ${nonPrefPostChance}, Random value: ${r}, Post on preferred day(s)?: ${
            r > nonPrefPostChance ? true : false
        }`
    );
    ...
    if (!calendar.preferredPostingDays.includes(rPrefDay)) {
        logger.warn(
            `Nope! ${format(rPrefDay, 'PPPP')} is not within ${
        ...
        if (
            ...
            if (r > nonPrefPostChance) {
                ...
                logger.info(
                    `Selected new posting date (pref): ${format(
                        setDay,
                        'PPPPpppp'
                    )}, generated from preferredPostingDays{}.`
                );
                return setDay;
            } else {
                rAnyDay =
                ...
                logger.info(
                    `Selected new posting date (any): ${format(
                        setDay,
                        'PPPPpppp'
                    )} from availablePostingDays{}.`
                );
                return setDay;
            }
        } else {
