...
const aidaInit = async () => {
    ...
    logger.info(
        `Display config file contents: ${JSON.stringify(
            readFromConfig(conf.default.aida)
        )}${JSON.stringify(readFromConfig(conf.default.calendar))}`
    );
    ...
    try {
        ...
        for (let x = 0; x < categories.length; x++) {
            ...
            logger.info(
                `Successfully imported tweet category from: ${categories[x]}! Let's start tweeting!`
            );
        }
    }
    ...
    if (readFromConfig(conf.default.aida.requireFinalApproval)) {
        ...
        if (choice == 'y')
            logger.info(
                'Great! Aida will begin posting using the above schedule! Leave the rest to her! 🤩'
            );
    }
};
